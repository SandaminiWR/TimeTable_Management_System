...
public class AddSession extends JFrame implements ActionListener, MouseListener{
	...
	public AddSession() {
		...
		panel_3_1.setBounds(86, 39, 677, 679);
		...
		btnSave_Insert.setBounds(69, 561, 146, 32);
		...
		btnReset_Insert.setBounds(271, 561, 146, 32);
		...
		btnBack_Insert.setBounds(472, 561, 146, 32);
	...
	public void refreshTableUpdate() {
		...
		/*try {
			while(rs.next()) {
				...
				tableModel.addRow(new Object[] {id,lecture1,lecture2, tag, room, maingroup, subgroup, subject,subjectCode, noOfStudent , day, timeslot});
			}
			
			this.tableLecSelecInsert.setModel(tableModel);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			System.out.println("refreshed table error");
			e.printStackTrace();
		}*/
		//refreshComboBox();
				...
				try {
					...
					if(x==2) {
						...
						if(lecFound == true) {
							...
							tableModel.addRow(new Object[] {id,Name});
							this.table_selec_lec_update.setModel(tableModel);
							x++;
						}
					}else {
						tableModel.addRow(new Object[] {id,Name});
	...
	public void updateSession(int ID, String lec1, String lec2, String subject, String subjectCode, String Tag, String Room, String Maingroup, String Subgroup, String Day, String timeslot, int noOfStudent) {
		...
		if( !( session.getLecture1().equals(lec1) ) ) {			
			changeMsg += lec1+"  ";
			session.setLecture1(lec1);
		}
		if( !( session.getLecture2().equals(lec2) ) ) {			
			changeMsg += lec2;
			session.setLecture1(lec2);
		}			
		if( !( session.getSub().equals(subject) ) ) {			
			changeMsg += "\n" + subject;
			session.setSub(subject);;
		}
		if( !( session.getTag().equals(Tag) ) ) {			
			changeMsg += "\n" + Tag;
			session.setTag(Tag);
		}
		if( !( session.getMaingroup().equals(Maingroup) ) ) {			
			changeMsg += "\n" + Maingroup;
			session.setMaingroup(Maingroup);
		}
		if( !( session.getSubgroup().equals(Subgroup) ) ) {			
			changeMsg += "." + Subgroup;
			session.setSubgroup(Subgroup);
		}
		if( !( session.getNoOfStudent() == noOfStudent )) {			
			changeMsg += "\n" + noOfStudent;
			session.setNoOfStudent(noOfStudent);
		}
		...
		if( confirm == 0 ) {			
			try {				
				this.ses.updateSession(session);
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource() == table_Update ) {
			...
			System.out.print(session.getLecture1());
			try {
